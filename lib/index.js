...
var runTests = function(runner, browser) {
  var cmds = [
    'phantomjs',
  ...
  exec(cmds.join(' '), function(err, stdout, stderr) {
    ...
    if(err !== null){
      console.error(err);
      process.exit(err.code); // pass on non zero status code
    }
  });
}

var createRunner = function(html, browser) {
  var runner = path.join(process.cwd(), 'runner.html');

  fs.writeFile(runner, html, function(err, data, error) {
    runTests(runner, browser);
  });
}
